...
layui.define(["element", "jquery"], function (exports) {
    ...
    layuimini = new function () {
        ...
        this.init = function (url) {
            ...
            $.getJSON(url, function (data, status) {
                if (data == null) {
                    ...
                    layuimini.initClear(clearUserCacheUrl);
                    layuimini.initTab();
                }
            }).fail(function () {
         ...
         * @param url
         */
        this.initLeftMenu = function (url) {
            ...
            $.each(data, function (key, val) {
                ...
                $.each(data, function (index, menu) {
                    ...
                    if (menu.children != undefined && menu.children.length>0) {
                        ...
                        var buildChildHtml = function (html, child, menuParameId) {
                            ...
                            $.each(child, function (childIndex, childMenu) {
                                ...
                                if (childMenu.children != undefined && childMenu.children.length>0) {
                                    html += '<a href="javascript:;" class="layui-menu-tips" ><i class="fa ' + childMenu.icon + '"></i><span class="layui-left-nav"> ' + childMenu.title + '</span></a>';
                                    html = buildChildHtml(html, childMenu.children, menuParameId);
                                } else {
                                    html += '<a href="javascript:;" class="layui-menu-tips" data-type="tabAdd"  data-tab-mpi="m-p-i-' + menuParameId + '" data-tab="' + childMenu.href + '" target="_self"><i class="fa ' + childMenu.icon + '"></i><span class="layui-left-nav"> ' + childMenu.title + '</span></a>\n';
                                    menuParameId++;
                        ...
                        leftMenuHtml += '<a href="javascript:;" class="layui-menu-tips"  data-type="tabAdd" data-tab-mpi="m-p-i-' + menuParameId + '" data-tab="' + menu.href + '" target="_self"><i class="fa ' + menu.icon + '"></i><span class="layui-left-nav"> ' + menu.title + '</span></a>\n';
                        menuParameId++;
                    }
                    leftMenuHtml += '</li>\n';
                });
        ...
        // 判断是否清理服务端
            $.get({
                ...
                success: function (res) {
                    layer.close(loading);
                    if(res.success){
                        return layuimini.msg_error(res.message);
                    }else{
